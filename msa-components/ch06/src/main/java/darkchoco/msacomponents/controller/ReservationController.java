...
import darkchoco.msacomponents.exception.FileDownloadException;
...
public class ReservationController {
    ...
    public List<Long> getReservationsByPaging(@PathVariable Long hotelId,
                                              ...
                                              Pageable pageable) {  // Spring은 자동으로 page, size, sort 파라미터 값을 mapping한 Pageable 객체를 주입한다.
        System.out.println("Page param : " + pageable.getPageNumber());
        System.out.println("Size param : " + pageable.getPageSize());
        pageable.getSort().stream()
                .forEach(order -> System.out.println("Sort param : " + order.getProperty() + " : " + order.getDirection()));
